...
// helper.js can be found on https://github.com/cloggy45/Gekko-Bot-Resources/tree/master/gekko
	...
	var helper = require('./indicators/helper.js');;
...
method.init = function(candle) {
	...
	this.input = 'candle';
	var customaoroonSettings = this.settings.parameters;
	...
	this.addTulipIndicator('myAroon', 'aroon', customaoroonSettings );
}
...
// Based on the newly calculated information, check if we should update or not.
	...
	if(aroonUp > this.settings.thresholds.bullup && aroonDown < this.settings.thresholds.bulldown && this.trend.state !== 'short') {
		this.trend.state = 'short';
		this.advice('short');
	}
	
	 //going green = Bull = BUYYYYY
	if(aroonDown > this.settings.thresholds.beardown && aroonUp < this.settings.thresholds.bearup && this.trend.state !== 'long') {
