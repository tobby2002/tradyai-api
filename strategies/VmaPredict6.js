...
strat = {
    ...
    init: function () {
        ...
        var initthistoml = { Magic: 0, MedmiumOut: 0, ShortSight: 0, DownHill : 0, BullsEye: 0, DayStop: 0, HighShot: 0 };
    ...
    check: function (candle) {
        ...
        //this.After.proceed happens after a stoploss
        //                   after that we should see at least 3 up candles and then a hammer
        //                   often big price falls are problematic times we try to wait when it happens sit on our cash
        //                   not that long though, the idea is that after this markets have stabelized again.
        //                   repeated deep falls where a problem in the past.




        if (!this.Gotcoins) {
            if (!this.DayStop && this.After.proceed && vmaMedium > maMedium && maShort < maMedium && predictShort > maShort) { this.long('Uptopica buy      ', this.color.White) }
